 ...
 * Email: kaushikdontula@gmail.com
 */
...
{
    //creates new linked list with dynamic memory of struct list
...
void list_free(struct list* list)
{
    ...
    //loops through list and frees until current == NULL (list is over)
	while(current != NULL){
		temp = current->next;
		free(current);
		current = temp;
    ...
    //finally frees list and sets head to NULL
	list->head = NULL;
    free(list);
...
{
    //creates new node and attaches it back to linked list with the value passed into function 
    struct node* tempnext = malloc(sizeof(struct node));

    //adds the value
    tempnext->val = val;

    //makes sure its at the front of the list
    tempnext->next = list->head;
    list->head = tempnext;
...
void list_insert_end(struct list* list, void* val)
{
    ...
    //create new dynamic memory node
    struct node* temp_next = malloc(sizeof(struct node));

    //initializing the new node 
    temp_next->val = val; 
    temp_next->next = NULL;

    //creating trailer node to follow
    struct node *trailer = list->head; 
    
    //if list is empty add node to head ptr
    if(list->head == NULL){
        list->head = temp_next;
        return;  
    }
    ...
    //adding the new node to tail
    trailer->next = temp_next; 
...
{
    //creating a temp head node and temp trailer node
    struct node* t_head = list->head;
    struct node* before = t_head->next;


    //if the val is in the first link of the list remove it
    if(cmp(val,t_head->val) == 0){
        list->head = t_head->next;
        free(t_head);
        return;
    }
    ...
    //traverse through list until the value is found
        ...
        while(t_head != NULL && cmp(val,t_head->val) != 0){
            before = t_head;
            t_head = t_head->next;
        }

        //once value is found free memory and set it to NULL
        before->next = t_head->next;
        free(t_head);
        t_head = NULL;
    }
    return;
...
void list_remove_end(struct list* list)
{
    ...
    //creating 2 temp nodes one head and one trailer
    struct node* temphead = list->head;
    struct node* infront = temphead->next;

    //if the list is not empty
    if(temphead !=NULL){

        //traverse through the list until the last value
        while(infront->next != NULL){
            infront = infront->next;
            temphead = temphead->next;
        }

        //free the last value and set it to NULL
        free(temphead->next);
        temphead->next = NULL;
    }
    return;
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b)){
    ...
    //if the value is in the first link of the list return 0 as the position
    if ((temp_head != NULL) && (cmp(val, temp_head->val) == 0)){
        return 0;
    }
    ...
    if (temp_head != NULL){
        ...
        while(temp_head->next != NULL){
            ...
            //if the value is found return the position +1 since we are one behind
            if (cmp(val, temp_head->val) == 0){
                return position+1;
            }

            //otherwise add to the position and go through loop again
            position++;
        }
    }
...
{
    //initializing 3 nodes current, before, after
    struct node *after = NULL;
    struct node *before = NULL;
    struct node *currentindex = list->head;

    //traverse through list until tail
    while(currentindex != NULL){

        //switch contents over
        after = currentindex->next;
        currentindex->next = before;
        before = currentindex;
        currentindex = after;
    }

    //make the head the before node
    list->head = before;
