...
{
    //allocates dynamic memory to the dynamic array using struct dynarray
    struct dynarray* da = malloc(sizeof(struct dynarray));
    
    //initializing size and capacity
...
{
    //free the data in the dynamic array
    free(da->data);
...
void dynarray_insert(struct dynarray* da, void* val)
{
    ...
    if(da->size < da->capacity){
        ...
        //the new value will be added to the end and size will increase by one
        da->data[da->size] = val;
        da->size++;
        
    }
    //else if the size is equal to the capacity
    else if(da->size == da->capacity){

        //double the capacity of the dynamic array
        da->capacity = da->capacity*2;

        //creates new dynamic array with new capacity
        void** temp = malloc(da->capacity * sizeof(void*));

        //copy over old elements of dynamic array
        for(int i=0; i<da->size; i++){
             temp[i] = da->data[i];
        }

        //adds the new element to the back index of the array and increases size by one
        temp[da->size] = val;
        da->size++;

        //free the old array and set it to NULL
        free(da->data);
        da->data = NULL;

        //attach the new dynamic array back to the old pointer
        da->data = temp;
    }
...
void dynarray_remove(struct dynarray* da, int idx)
{
    ...
    //the size is now decreased by one
    da->size=da->size-1;
    return;
